...
#define _CONTROL_NUM 100
    ...
    IndexGraph::IndexGraph(const size_t dimension, const size_t n, efanna2e::Metric m, Index *initializer)
            : Index(dimension, n, m),
...
#pragma omp parallel for default(shared) schedule(dynamic, 100)
    ...
    void IndexGraph::update(const efanna2e::Parameters &parameters) {
        unsigned S = parameters.Get<unsigned>("S_knn");
        unsigned R = parameters.Get<unsigned>("R_knn");
        unsigned L = parameters.Get<unsigned>("L_knn");
            ...
            for (unsigned l = 0; l < nnhd.M; ++l) {
                ...
                if (nn.flag) {
                    ...
                    if (nn.distance > nhood_o.pool.back().distance) {
                        knn_efanna2e::LockGuard guard(nhood_o.lock);
...
#pragma omp parallel for
    ...
    void IndexGraph::NNDescent(const efanna2e::Parameters &parameters) {
        unsigned iter = parameters.Get<unsigned>("iter_knn");
...
#pragma omp parallel for
        for (unsigned i = 0; i < c.size(); i++) {
            std::vector<knn_efanna2e::Neighbor> tmp;
            for (unsigned j = 0; j < N; j++) {
                float dist = distance_->compare(data_ + c[i] * dimension_, data_ + j * dimension_, dimension_);
                tmp.push_back(knn_efanna2e::Neighbor(j, dist, true));
            }
    ...
    void IndexGraph::InitializeGraph(const efanna2e::Parameters &parameters) {

        const unsigned L = parameters.Get<unsigned>("L_knn");
        const unsigned S = parameters.Get<unsigned>("S_knn");

        graph_.reserve(nd_);
        std::mt19937 rng(rand());
        for (unsigned i = 0; i < nd_; i++) {
            graph_.push_back(knn_efanna2e::nhood(L, S, rng, (unsigned) nd_));
        }
#pragma omp parallel for
        for (unsigned i = 0; i < nd_; i++) {
            ...
            for (unsigned j = 0; j < S; j++) {
                ...
                graph_[i].pool.push_back(knn_efanna2e::Neighbor(id, dist, true));
            }
    ...
    void IndexGraph::InitializeGraph_Refine(const efanna2e::Parameters &parameters) {
...
#pragma omp parallel for
    ...
    void IndexGraph::RefineGraph(const float *data, const efanna2e::Parameters &parameters) {
        data_ = data;
        ...
        unsigned K = parameters.Get<unsigned>("K_knn");
        for (unsigned i = 0; i < nd_; i++) {
            ...
            std::vector<knn_efanna2e::Neighbor>().swap(graph_[i].pool);
            std::vector<unsigned>().swap(graph_[i].nn_new);
            std::vector<unsigned>().swap(graph_[i].nn_old);
            std::vector<unsigned>().swap(graph_[i].rnn_new);
            std::vector<unsigned>().swap(graph_[i].rnn_new);
        }
        std::vector<knn_efanna2e::nhood>().swap(graph_);
        has_built = true;

    }

//dabddbd8
    void IndexGraph::Build(size_t n, const float *data, const efanna2e::Parameters &parameters) {
    ...
    void IndexGraph::Search(
            ...
            const float *x,
            size_t K,
            const efanna2e::Parameters &parameter,
            unsigned *indices) {
        const unsigned L = parameter.Get<unsigned>("L_search");

        std::vector<knn_efanna2e::Neighbor> retset(L + 1);
        ...
        for (unsigned i = 0; i < L; i++) {
            ...
            retset[i] = knn_efanna2e::Neighbor(id, dist, true);
        }
        ...
        while (k < (int) L) {
            ...
            if (retset[k].flag) {
                ...
                for (unsigned m = 0; m < final_graph_[n].size(); ++m) {
                    ...
                    knn_efanna2e::Neighbor nn(id, dist, true);
    ...
    void IndexGraph::parallel_graph_insert(unsigned id, knn_efanna2e::Neighbor nn, LockGraph &g, size_t K) {
        knn_efanna2e::LockGuard guard(g[id].lock);
    ...
    void IndexGraph::GraphAdd(const float *data, unsigned n_new, unsigned dim, const efanna2e::Parameters &parameters) {
        data_ = data;
        data += nd_ * dimension_;
        assert(final_graph_.size() == nd_);
...
#pragma omp for
            for (unsigned i = 0; i < n_new; i++) {
                std::vector<knn_efanna2e::Neighbor> res;
                get_neighbor_to_add(data + i * dim, parameters, graph_tmp, rng, res, n_new);

                for (unsigned j = 0; j < K; j++) {
                    parallel_graph_insert(i + (unsigned) nd_, res[j], graph_tmp, K);
                    parallel_graph_insert(res[j].id, knn_efanna2e::Neighbor(i + (unsigned) nd_, res[j].distance, true),
                                          graph_tmp, K);
                }
    ...
    void IndexGraph::get_neighbor_to_add(const float *point,
                                         ...
                                         LockGraph &g,
                                         std::mt19937 &rng,
                                         std::vector<knn_efanna2e::Neighbor> &retset,
                                         unsigned n_new) {
        ...
        while (k < (int) L) {
            ...
            if (retset[k].flag) {
                ...
                knn_efanna2e::LockGuard guard(g[n].lock);//lock start
    ...
    void IndexGraph::compact_to_Lockgraph(LockGraph &g) {
        ...
        for (unsigned i = 0; i < final_graph_.size(); i++) {
            ...
            for (unsigned j = 0; j < final_graph_[i].size(); j++) {
                ...
                g[i].pool.push_back(knn_efanna2e::Neighbor(final_graph_[i][j], dist, true));
            }
    ...
    IndexGraph::CompactGraph IndexGraph::ReturnFinalGraph()
    {
        return final_graph_;
    }
}
